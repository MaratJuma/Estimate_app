...
from core.services.estimates import (
    ...
    get_next_contract_estimate_number,
)
...
class EstimateServicesTestCase(TenantTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_next_contract_estimate_number_returns_next_number(self):
        with tenant_context(self.tenant):
            self.assertEqual(get_next_contract_estimate_number('DOG-001'), 1)
            ...
            self.assertEqual(get_next_contract_estimate_number('DOG-001'), 3)
    ...
    def test_duplicate_estimate_creates_copy_with_actual_cost_prices_and_new_owner(self):
        with tenant_context(self.tenant):
            ...
            self.assertEqual(new_estimate.contract_number, 'DOG-001')
            self.assertEqual(new_estimate.contract_estimate_number, 2)
    ...
    def test_duplicate_estimate_with_new_contract_starts_from_one(self):
        with tenant_context(self.tenant):
            source_estimate = create_estimate_with_first_day(
                client_name='Клиент',
                comment='Исходная смета',
            ...
            new_estimate = duplicate_estimate(
                source_estimate,
                user=duplicator,
                contract_number='DOG-999',
            )

            self.assertEqual(new_estimate.contract_number, 'DOG-999')
            self.assertEqual(new_estimate.contract_estimate_number, 1)